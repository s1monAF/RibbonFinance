...
contract MarginCalculator is Ownable {
    ...
    mapping(bytes32 => mapping(uint256 => uint256))
        internal maxPriceAtTimeToExpiry;
    ...
    constructor(address _oracle) {
        require(
            _oracle != address(0),
            "MarginCalculator: invalid oracle address"
        );
     ...
     */
    function setCollateralDust(address _collateral, uint256 _dust)
        external
        onlyOwner
    {
        require(
    ...
    function setUpperBoundValues(
        ...
        require(
            _timesToExpiry.length > 0,
            "MarginCalculator: invalid times to expiry array"
        );
        require(
            _timesToExpiry.length == _values.length,
        ...
        require(
            (expiryArray.length == 0) ||
                (_timesToExpiry[0] > expiryArray[expiryArray.length.sub(1)]),
        ...
        for (uint256 i = 0; i < _timesToExpiry.length; i++) {
            ...
            if (i.add(1) < _timesToExpiry.length) {
                require(
                    _timesToExpiry[i] < _timesToExpiry[i.add(1)],
    ...
    function updateUpperBoundValue(
        ...
        uint256 oldMaxPrice = maxPriceAtTimeToExpiry[productHash][
            _timeToExpiry
        ];
     ...
     * @return dust amount
     */
    function getCollateralDust(address _collateral)
        external
        view
        returns (uint256)
    {
    ...
    function getNakedMarginRequired(
        ...
        FPI.FixedPointInt memory shortAmount = FPI.fromScaledUint(
            _shortAmount,
            BASE
        );
        // scale short strike from 1e8 to 1e27
        FPI.FixedPointInt memory shortStrike = FPI.fromScaledUint(
            _strikePrice,
            BASE
        );
        ...
        returns (uint256)
    {
        require(
            _otoken != address(0),
            "MarginCalculator: Invalid token address"
        );
        ...
        require(
            ...
            "MarginCalculator: Otoken not expired yet"
        );
        ...
        FPI.FixedPointInt
            memory cashValueInCollateral = _convertAmountOnExpiryPrice(
                cashValueInStrike,
                strikeAsset,
                collateral,
                expiry
        ...
        uint256 collateralDecimals = uint256(
            IERC20Detailed(collateral).decimals()
        );
        ...
        )
    {
        ...
        ShortScaledDetails memory shortDetails = ShortScaledDetails({
            ...
            shortStrike: FPI.fromScaledUint(
    ...
    function getExcessCollateral(
        ...
        if (!vaultDetails.hasShort && !vaultDetails.hasLong) {
            uint256 amount = vaultDetails.hasCollateral
                ? _vault.collateralAmounts[0]
                : 0;
            return (amount, true);
        }
    ...
    function _getExpiredCashValue(
        ...
        FPI.FixedPointInt memory strikePrice = FPI.fromScaledUint(
            _strikePrice,
            BASE
        );
        ...
        if (block.timestamp < otokenDetails.otokenExpiry) {
            ...
            if (_vaultDetails.vaultType == 1) {
                ...
                if (otokenDetails.isPut) {
                    FPI.FixedPointInt
                        memory strikeNeeded = _getPutSpreadMarginRequired(
                            shortAmount,
                            longAmount,
                            shortStrike,
                            longStrike
                    ...
                    FPI.FixedPointInt
                        memory underlyingNeeded = _getCallSpreadMarginRequired(
                            shortAmount,
                            longAmount,
                            shortStrike,
                            longStrike
    ...
    function _getNakedMarginRequired(
        ...
        FPI.FixedPointInt memory optionUpperBoundValue = _findUpperBoundValue(
            _productHash,
            _shortExpiryTimestamp
        );
        // convert spot shock value of this product to FixedPointInt (already scaled by 1e27)
        FPI.FixedPointInt memory spotShockValue = FPI.FixedPointInt(
            int256(spotShock[_productHash])
        );
        ...
        if (_isPut) {
            ...
            b = FPI.max(
                _strikePrice.sub(spotShockValue.mul(_underlyingPrice)),
                ZERO
            );
            marginRequired = optionUpperBoundValue.mul(a).add(b).mul(
                _shortAmount
            );
        } else {
            FPI.FixedPointInt memory one = FPI.fromScaledUint(
                1e27,
                SCALING_FACTOR
            );
            a = FPI.min(
            ...
            b = FPI.max(
                one.sub(_strikePrice.mul(spotShockValue).div(_underlyingPrice)),
                ZERO
            );
            marginRequired = optionUpperBoundValue.mul(a).add(b).mul(
                _shortAmount
    ...
    function _findUpperBoundValue(
        ...
        for (uint8 i = 0; i < timesToExpiry.length; i++) {
            if (timesToExpiry[i] >= optionTimeToExpiry)
                return
                    FPI.fromScaledUint(
                        maxPriceAtTimeToExpiry[_productHash][timesToExpiry[i]],
                        SCALING_FACTOR
                    );
        }

        return FPI.fromScaledUint(0, SCALING_FACTOR);
    }
    ...
    function _getPutSpreadMarginRequired(
        ...
        return
            FPI.max(
                _shortAmount.mul(_shortStrike).sub(
                    _longStrike.mul(FPI.min(_shortAmount, _longAmount))
                ),
                ZERO
            );
    }
    ...
    function _getCallSpreadMarginRequired(
         ...
         *                             long strike
         */
        FPI.FixedPointInt memory firstPart = _longStrike
            .sub(_shortStrike)
            .mul(_shortAmount)
            .div(_longStrike);
         ...
         */
        FPI.FixedPointInt memory secondPart = FPI.max(
            _shortAmount.sub(_longAmount),
            ZERO
        );
    ...
    function _convertAmountOnLivePrice(
        ...
        return
            _amount.mul(FPI.fromScaledUint(priceA, BASE)).div(
                FPI.fromScaledUint(priceB, BASE)
            );
    }
    ...
    function _convertAmountOnExpiryPrice(
        ...
        (uint256 priceA, bool priceAFinalized) = oracle.getExpiryPrice(
            _assetA,
            _expiry
        );
        (uint256 priceB, bool priceBFinalized) = oracle.getExpiryPrice(
            _assetB,
            _expiry
        );
        require(
            priceAFinalized && priceBFinalized,
    ...
    function _getDebtPrice(
        ...
        if (auctionElapsedTime >= AUCTION_TIME) {
...

            {
                ...
                FPI.FixedPointInt memory fixedOracleDeviation = FPI
                    .fromScaledUint(oracleDeviation, SCALING_FACTOR);

                if (_isPut) {
                    startingPrice = FPI.max(
                        _cashValue.sub(fixedOracleDeviation.mul(_spotPrice)),
                        ZERO
                    );
                } else {
                    startingPrice = FPI
                        .max(
                            _cashValue.sub(
                                fixedOracleDeviation.mul(_spotPrice)
                            ),
                            ZERO
                        )
                        .div(_spotPrice);
            ...
            FPI.FixedPointInt memory auctionElapsedTimeFixedPoint = FPI
                .fromScaledUint(auctionElapsedTime, 18);
            // store AUCTION_TIME in a FixedPointInt (already scaled by 1e27)
            FPI.FixedPointInt memory auctionTime = FPI.fromScaledUint(
                AUCTION_TIME,
                18
            );
            ...
            price = startingPrice.add(
                (endingPrice.sub(startingPrice))
                    .mul(auctionElapsedTimeFixedPoint)
                    .div(auctionTime)
            );
    ...
    function _getVaultDetails(
        ...
        if (vaultDetails.hasLong) {
            IOtoken long = IOtoken(_vault.longOtokens[0]);
            (
            ...
            vaultDetails.longCollateralDecimals = uint256(
                IERC20Detailed(vaultDetails.longCollateralAsset).decimals()
            );
        }

        // get vault short otoken if available
        if (vaultDetails.hasShort) {
            IOtoken short = IOtoken(_vault.shortOtokens[0]);
            (
                vaultDetails.shortCollateralAsset,
                vaultDetails.shortUnderlyingAsset,
                vaultDetails.shortStrikeAsset,
            ...
            vaultDetails.shortCollateralDecimals = uint256(
                IERC20Detailed(vaultDetails.shortCollateralAsset).decimals()
            );
        }

        if (vaultDetails.hasCollateral) {
            vaultDetails.collateralDecimals = uint256(
                IERC20Detailed(_vault.collateralAssets[0]).decimals()
            );
        }
    ...
    function _getExpiredSpreadCashValue(
        ...
        return
            _shortCashValue.mul(_shortAmount).sub(
                _longCashValue.mul(_longAmount)
            );
    }
     ...
     */
    function _isNotEmpty(address[] memory _assets)
        internal
        pure
        returns (bool)
    {
     ...
     */
    function _isMarginableLong(
        MarginVault.Vault memory _vault,
        VaultDetails memory _vaultDetails
    ) internal pure returns (bool) {
        if (_vaultDetails.vaultType == 1)
            require(
                !_vaultDetails.hasLong,
     ...
     */
    function _isMarginableCollateral(
        MarginVault.Vault memory _vault,
        VaultDetails memory _vaultDetails
    ) internal pure returns (bool) {
        ...
        if (_vaultDetails.hasShort) {
            ...
            isMarginable =
                _vaultDetails.longCollateralAsset == _vault.collateralAssets[0];
        }
    ...
    function _getCashValue(
        ...
        if (_isPut)
            return
                _strikePrice.isGreaterThan(_underlyingPrice)
                    ? _strikePrice.sub(_underlyingPrice)
                    : ZERO;

        return
            _underlyingPrice.isGreaterThan(_strikePrice)
                ? _underlyingPrice.sub(_strikePrice)
                : ZERO;
    }
