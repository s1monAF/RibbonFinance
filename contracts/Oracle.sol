...
import "./interfaces/IOpynPricer.sol";
...
contract Oracle is IOracle, Ownable {
    ...
    function migrateOracle(
        ...
        for (uint256 i; i < _expiries.length; i++) {
            storedPrice[_asset][_expiries[i]] = Price(
                _prices[i],
                block.timestamp
            );
        }
    ...
    function getPrice(address _asset) external view returns (uint256) {
        ...
        if (price == 0) {
            ...
            price = IOpynPricer(assetPricer[_asset]).getPrice();
        }
        ...
        returns (uint256, uint256)
    {
        ...
        if (price == 0) {
            ...
            (price, timestamp) = IOpynPricer(assetPricer[_asset])
        ...
        returns (bool)
    {
        ...
        if (price == 0) {
            ...
            Price memory _price = storedPrice[_asset][_expiryTimestamp];
            if (_price.timestamp == 0) {
                return false;
            }
            ...
            return block.timestamp > _price.timestamp + disputePeriod;
        }
